...
function goPage(pageId) {
  ...
  // 需要登入的頁面（分享區 feed 允許訪客直接瀏覽）
  if ((pageId === 'home' || pageId === 'post') && !currentUser) {
    goPage('feed');
    showToast('🔑', '請先登入', '登入後才能使用此功能', 'warning');
    return;
  }
...
function refreshFeed() {
  ...
  // 動態渲染右上角：訪客顯示登入按鈕，已登入顯示頭貼
  const feedNavUser = document.getElementById('feedNavUser');
  if (feedNavUser) {
    if (currentUser) {
      feedNavUser.innerHTML = `<div class="nav-avatar" id="navAvatarFeed" onclick="goPage('home')" style="cursor:pointer"></div>`;
      renderAvatar(document.getElementById('navAvatarFeed'), currentUser.avatar, currentUser.name);
    } else {
      feedNavUser.innerHTML = `<button class="btn btn-primary btn-sm" onclick="goPage('auth')" style="white-space:nowrap">🔑 登入 / 註冊</button>`;
    }
  }
}
